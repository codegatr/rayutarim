<?php
/**
 * RAYU TARIM MAKINELERI - Public katalog verisi
 */
declare(strict_types=1);

function ru_catalog_categories(): array
{
    try {
        $rows = ru_fetch_all('SELECT * FROM ' . ru_t('product_categories') . ' WHERE is_active = 1 ORDER BY sort_order ASC, title ASC');
        if ($rows) {
            $out = [];
            foreach ($rows as $row) {
                $out[(string)$row['slug']] = [
                    'id' => (int)$row['id'],
                    'title' => (string)$row['title'],
                    'summary' => (string)($row['summary'] ?? ''),
                    'icon' => (string)($row['icon'] ?? ''),
                ];
            }
            return $out;
        }
    } catch (Throwable) {
    }
    return [
        'toprak-isleme' => [
            'title' => 'Toprak İşleme',
            'summary' => 'Farklı üreticilerden pulluk, kültivatör, çizel, merdane ve tarla hazırlık ekipmanları.',
            'icon' => 'M4 18c5-8 10-11 16-12-2 7-1 12 3 16 4-4 1-9 2-16 8Z',
        ],
        'traktor-ekipman' => [
            'title' => 'Traktör & Güç Ekipmanları',
            'summary' => 'Traktör, ön yükleyici, kuyruk mili ekipmanları ve güç aktarım çözümleri.',
            'icon' => 'M3 13h2l2-4h10l2 4h2a3 3 0 0 1 3 3v2h2a3 3 0 0 1 3-3z',
        ],
        'ekim-dikim' => [
            'title' => 'Ekim ve Dikim',
            'summary' => 'Hassas ekim makineleri, mibzerler, dikim makineleri ve gübre üniteleri.',
            'icon' => 'M12 3c4 4 6 8 6 12a6 6 0 0 1-12 0c0-4 2-8 6-12Z',
        ],
        'ilaclama' => [
            'title' => 'Zirai İlaçlama',
            'summary' => 'Pülverizatör, atomizör, drone destekli uygulama ve doğru ilaçlama ekipmanları.',
            'icon' => 'M7 3h10v4l-2 3v8a3 3 0 0 1-6 0v-8L7 7V3Z',
        ],
        'hasat' => [
            'title' => 'Hasat & Paketleme',
            'summary' => 'Hasat makineleri, balya, römork, taşıma ve paketleme çözümleri.',
            'icon' => 'M4 17h16v2H4v-2zm2-4h12m-9 4h6m-6 4h4z',
        ],
        'yedek-parca' => [
            'title' => 'Yedek Parça ve Servis',
            'summary' => 'Markaya göre yedek parça, sarf malzeme, servis paketi ve bakım planlama.',
            'icon' => 'M19 13a7 7 0 1 1-8-8l2 2-2 4 4-2 2 2a7 7 0 0 1 2 4Z',
        ],
    ];
}

function ru_catalog_products(): array
{
    try {
        $rows = ru_fetch_all('SELECT p.*, c.slug category_slug FROM ' . ru_t('products') . ' p LEFT JOIN ' . ru_t('product_categories') . ' c ON c.id = p.category_id WHERE p.is_active = 1 ORDER BY p.is_featured DESC, p.sort_order ASC, p.name ASC');
        if ($rows) {
            return array_map('ru_catalog_product_from_row', $rows);
        }
    } catch (Throwable) {
    }
    return [
        [
            'category' => 'toprak-isleme',
            'name' => 'Ağır Tip Çizel',
            'brand' => 'Çoklu marka',
            'badge' => 'Karşılaştırmalı teklif',
            'summary' => 'Derin patlatma için farklı üretici seçenekleri, HP uyumu ve teslimat karşılaştırması.',
            'specs' => ['9-13 ayak', '70-120 HP', 'Opsiyonel merdane'],
        ],
        [
            'category' => 'traktor-ekipman',
            'name' => 'Traktör & Ön Yükleyici Paketi',
            'brand' => 'Bayi ağı',
            'badge' => 'Yeni / stok',
            'summary' => 'Bölgesel bayi stokları, finansman seçenekleri ve ekipman uyumu birlikte değerlendirilir.',
            'specs' => ['50-120 HP', 'Ataşman seçimi', 'Teslimat planı'],
        ],
        [
            'category' => 'toprak-isleme',
            'name' => 'Diskli Goble',
            'brand' => 'Yerli üretici',
            'badge' => 'Saha hazırlığı',
            'summary' => 'Anız parçalama ve homojen karıştırma için uygun marka ve disk geometrisi seçimi.',
            'specs' => ['20-32 disk', 'Hidrolik ayar', 'Ağır hizmet rulman'],
        ],
        [
            'category' => 'ekim-dikim',
            'name' => 'Pnömatik Hassas Ekim',
            'brand' => 'Seçili tedarikçi',
            'badge' => 'Yüksek verim',
            'summary' => 'Tohum aralığı kontrolü, gübre ünitesi ve sezonluk kalibrasyon destekli marka seçimi.',
            'specs' => ['4-8 sıra', 'Vakum sistem', 'Gübre deposu'],
        ],
        [
            'category' => 'ilaclama',
            'name' => 'Asılır Tip Tarla Pülverizatörü',
            'brand' => 'İlaçlama ağı',
            'badge' => 'Bitki sağlığı',
            'summary' => 'Dengeli bom yapısı, nozul seçenekleri ve uygulama standardına göre teklif.',
            'specs' => ['600-1000 L', '12-16 m bom', 'Basınç regülatörü'],
        ],
        [
            'category' => 'hasat',
            'name' => 'Balya & Taşıma Çözümleri',
            'brand' => 'Hasat tedariki',
            'badge' => 'Sezon teslimi',
            'summary' => 'Balya, römork ve taşıma ekipmanlarında stok, servis ve yedek parça karşılaştırması.',
            'specs' => ['Balya', 'Römork', 'Servis kapsamı'],
        ],
        [
            'category' => 'yedek-parca',
            'name' => 'Sezon Bakım Paketi',
            'brand' => 'Parça tedariki',
            'badge' => 'Servis',
            'summary' => 'Aşınan parçalar, rulman, bıçak, hortum ve marka uyumlu saha servis planı.',
            'specs' => ['Hızlı sevk', 'Orijinal parça', 'Servis kaydı'],
        ],
    ];
}

function ru_catalog_products_by_category(string $category): array
{
    try {
        $rows = ru_fetch_all('SELECT p.*, c.slug category_slug FROM ' . ru_t('products') . ' p LEFT JOIN ' . ru_t('product_categories') . ' c ON c.id = p.category_id WHERE p.is_active = 1 AND c.slug = ? ORDER BY p.sort_order ASC, p.name ASC', [$category]);
        if ($rows) {
            return array_map('ru_catalog_product_from_row', $rows);
        }
    } catch (Throwable) {
    }
    return array_values(array_filter(
        ru_catalog_products(),
        static fn (array $product): bool => $product['category'] === $category
    ));
}

function ru_catalog_product_from_row(array $row): array
{
    $specs = json_decode((string)($row['specs_json'] ?? '[]'), true);
    return [
        'id' => (int)$row['id'],
        'category' => (string)($row['category_slug'] ?? ''),
        'slug' => (string)$row['slug'],
        'name' => (string)$row['name'],
        'brand' => (string)($row['brand'] ?? $row['supplier_name'] ?? 'RAYU tedarik ağı'),
        'badge' => (string)($row['badge'] ?? ''),
        'summary' => (string)($row['summary'] ?? ''),
        'description' => (string)($row['description'] ?? ''),
        'specs' => is_array($specs) ? $specs : [],
        'image' => (string)($row['image'] ?? ''),
    ];
}
